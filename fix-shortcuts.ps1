# Quick fix for shortcut creation in install.bat
# Run this if you already have the installer folder but shortcuts aren't working

$InstallerDir = ".\installer"

if (-not (Test-Path $InstallerDir)) {
    Write-Host "Error: installer folder not found. Run test-build.bat first." -ForegroundColor Red
    exit 1
}

Write-Host "Updating install.bat with fixed shortcut commands..." -ForegroundColor Green
...
Write-Host ""
Write-Host "Fixed! The install.bat file has been updated." -ForegroundColor Green
Write-Host ""
Write-Host "You can now:" -ForegroundColor Yellow
Write-Host "1. Test it: cd installer && install.bat" -ForegroundColor White
Write-Host "2. Or create ZIP: Compress-Archive -Path .\installer\* -DestinationPath RepairShopBilling.zip" -ForegroundColor White
